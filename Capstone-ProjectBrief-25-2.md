...
| (7) 과제 내용 요약 | 본 프로젝트는 소상공인 베이커리 점포의 **품절·폐기 문제 해결**을 목표로, POS 데이터와 외부 요인(날씨, 요일·공휴일, 유동인구, 프로모션)을 결합한 **품목별 수요 예측 시스템**을 개발한다.<br><br> • **문제 정의**: 기존 발주는 점주의 경험에 의존해 과잉 발주(폐기)·과소 발주(품절) 문제가 발생. 실제 점포 인터뷰를 통해 AI 기반 수요예측 서비스의 필요성과 수요를 검증.<br><br>
• **제안 내용**: CatBoost·HGBR 등 머신러닝 모델을 활용해 SKU 단위 판매량을 예측하고, 점포별 최적 모델을 선정. 결과는 CSV 리포트(주 1회)와 **웹 대시보드**로 제공.<br><br> • **주요 기능**: 데이터 전처리 자동화, 모델 학습 및 검증, 발주 가이드 제시, 시각화 대시보드.<br><br> • **협업 현황**: ㈜라포르엘과 협력하여 Randy’s Donuts 국내 4개 지점에 적용, R² 80% 이상의 예측 정확도를 달성.<br><br>본 시스템은 **데이터 수집 → 모델 학습 → 수요예측 운영 → 결과 제공**의 전 과정을 체계적으로 연결하여, 연구 단계를 넘어 실제 상용 서비스로 확장 가능한 기반을 갖추었다. |
